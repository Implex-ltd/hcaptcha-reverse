...
// Function to select a random vendor and renderer
function getRandomVendorRenderer() {
    // Get a random vendor from the keys of the vendorsRenderers object
    const vendors = Object.keys(GPUS);
    const randomVendor = vendors[Math.floor(Math.random() * vendors.length)];

    // Get the list of renderers for the selected vendor
    const rendererList = GPUS[randomVendor];

    // Get a random renderer from the list
    const randomRenderer = rendererList[Math.floor(Math.random() * rendererList.length)];

    // Return the selected vendor and renderer
...
var hsw = function () {
    ...
    */
    ...
    function generateHumanLikeScreenSize() {
        const commonScreenSizes = [
            { width: 1366, height: 768 },   // Standard laptop screen
            ...
            { width: 1440, height: 900 },   // Common desktop monitor
            { width: 1280, height: 800 },   // Common laptop screen
            { width: 1680, height: 1050 },  // Widescreen desktop monitor
            //{ width: 768, height: 1366 },  // Portrait smartphone
            //{ width: 375, height: 667 },   // Landscape smartphone
            //{ width: 800, height: 1280 },  // Portrait tablet
            //{ width: 600, height: 1024 },  // Landscape tablet
        ];

        const randomIndex = Math.floor(Math.random() * commonScreenSizes.length);
        return commonScreenSizes[randomIndex];
    }
    ...
    function appendJsonToMemory(pp) {
        ...
        // Convert the JSON string to inject to a Uint8Array
        ...
        // Calculate the required size after appending to_inject
        const requiredSize = currentSize + to_inject.length;

        // Grow the memory buffer to the required size
        M.memory.grow(Math.ceil((requiredSize - currentSize) / 65536));

        // Get a new reference to the memory buffer (since it may have been reallocated)
        const updatedBuffer = M.memory.buffer;

        // Create a Uint8Array view of the memory buffer
        const memoryView = new Uint8Array(updatedBuffer);

        // Copy the bytes of to_inject to the end of the memory buffer
        memoryView.set(to_inject, currentSize);

        //console.log('Appended JSON:', to_inject);
    ...
    var yI = Object.freeze({
        ...
        inject: function (len, ptr) {
            //  console.log('inject', ptr, len);

            try {
                let ndata = JSON.parse(__getStrFromWasm(ptr, len))
                //console.log(n)

                const screenSize = generateHumanLikeScreenSize();
                const { vendor, renderer } = getRandomVendorRenderer();
                ...
                let base_payload = JSON.stringify({
                    ...
                    "components": {
                        ...
                        "screen": {
                            "color_depth": screen_dim[5],
                            "pixel_depth": screen_dim[6],
                            "width": screen_dim[0],
                            "height": screen_dim[1],
                            "avail_width": screen_dim[2],
                            "avail_height": screen_dim[3]
                        },
                        "device_pixel_ratio": 1.0,
                        "has_session_storage": true,
                        "has_local_storage": true,
                        "has_indexed_db": true,
                        "web_gl_hash": "-1",//generateRandomString("6319351902351878741".length),
                        "canvas_hash": generateRandomString("6319351902351878741".length),
                        "has_touch": false,
                    ...
                    "fingerprint_events": [
                        ...
                        [
                            1403,
                            __EncryptStr("Europe/Paris")
                            //"[\"ZkMLCkCPPNa=YFBPZ2Z2\",\"11\",\"9\",\"QDLUNPCBIERSN\"]"
                        ],
                        [
                            3504,
                        ...
                        [
                            3501,
                            "[[\"navigation:newassets.hcaptcha.com\",14.099999999627471,28.5],[\"script:newassets.hcaptcha.com\",14.450000000186265,30.600000000558794],[\"xmlhttprequest:hcaptcha.com\",0,32.200000000186265]]"
                        ],
                        [
                            3503,
                            "14.099999999627471"
                        ...
                        [
                            2402,
                            JSON.stringify([vendor, renderer])
                        ...
                        [
                            3210,
                            "[241383851728,241383851728,null,null,4294705152,true,true,true,null]"
                        ],
                        [
                            3211,
                            "[\"MZGZM4uTM3IDOYqT\",\"1a\",\"3\",\"UHZXYHKBLGNQE\"]"
                        ],
                        [
                            702,
                ...
                // console.log(`JSON appended at ptr: ${data.ptr}, len: ${data.len}`);

            } catch (err) { console.log(err) }
        },
        getPtr: function () {
            // console.log('get jptr', jptr)
            return jptr
        },
        getLen: function () {
            //console.log('get jlen', jlen)
            return jlen
        },
